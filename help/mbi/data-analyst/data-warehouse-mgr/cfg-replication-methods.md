...
**알고 계십니까?**&#x200B;현재 데이터베이스에서 복제 방법을 지원할 `Incremental` 수 없는 경우에도 또는 `Single Auto Incrementing PK` 를 사용 `Modified At` 하도록 설정 하는 데이터베이스 ](../../best-practices/mod-db-inc-replication.md) 를 변경할 수 [ 있습니다.
...
이 메서드는 다음 기준을 충족 하는 테이블에서 새 데이터를 복제 하도록 설계 되었습니다.
...
메서드는 `Add Date` 메서드와 비슷하게 `Single Auto Incrementing Primary Key` 작동 합니다. 이 메서드는 테이블의 기본 키에 대해 정수를 사용 하는 대신 열을 사용 `timestamped` 하 여 새 행을 확인 합니다.

테이블에서 복제를 사용 하 `Add Date` 는 경우 데이터 웨어하우스에 동기화 된 최신 날짜 보다 큰 타임 스탬프가 지정 된 값을 검색 하 여 새 데이터를 검색 합니다. 예를 들어, 20/12/2015 09 :00: 00에서 마지막으로 실행 된 업데이트가 있는 경우, 타임 스탬프가이 값 보다 큰 행은 새 데이터로 표시 되 고 복제 됩니다.

>[!NOTE]
>
>`Modified At`메서드와 `Add Date` 달리 업데이트 된 정보에 대해서는 기존 행을 확인 하지 않습니다.
...
* 이후 업데이트 주기 간에 로컬 데이터베이스 테이블에서 만든 것 보다 더 많은 행이 삭제 됩니다.
* 열 값이 변경 되었지만 추가 행이 생성 되지 않습니다.
...
테이블에서 를 사용하는 경우 `Primary Key Batch` (PK 배치), 기본 키 값의 범위 내 행 또는 배치 수를 계산하여 새 데이터를 검색합니다. 일반적으로 정수에 사용 되는 것으로 생각 되는 경우에는 시스템에서 상수 범위를 정의할 수 있는 방식으로 텍스트 값을 주문할 수 균일.

예를 들어 업데이트가 실행 되 고 1에서 100 까지의 키 범위에 대 한 행 카운트를 수행 한다고 말할 수 있습니다. 이 업데이트에서 시스템은 37 행을 찾고 기록 합니다. 다음 업데이트에서 행 카운트는 1-100 범위에서 다시 실행 되 고 41 행을 찾습니다. 마지막 업데이트와 비교 되는 행의 수가 다르므로 시스템이 해당 범위 (또는 배치)를 더 자세히 검사 합니다.

이 메서드는 다음 기준을 충족 하는 테이블에서 데이터를 복제 하기 위한 것입니다.
...
| `Primary Key Batch Monitoring` | 느린 | 느린 | 예 | 예 | 예 | 예 |
